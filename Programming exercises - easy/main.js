...
//calculates average between numbers
let notas = [/*4.2, 7.9, 3.6, 0.9, 2.3*/ 20, 40, 70];
...
// Calculetes if value is prime or not
function epri(valor) {
...
//verifies if it is a identity matrix
let ident = [[5, 0, 0],
            [0, 5, 0], 
            [0, 0, 5]];
...
// Algorithm to find biggest number in array
let values = [9, 11, 13];

function find_big_num(values) {
    let bgNum = values[0];
...
//Algorithm that counts till 100 and prints Buzz if value is divisible by 5 and Fizz if divisible by 3
